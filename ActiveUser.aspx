...
<% 
    ...
    rndtext = Trim(Request.QueryString("rndtext"))
    If username = "" Or rndtext = "" Then
        FunctionClass.DisplayErr(weberr.parerr)
    ElseIf FunctionClass.IsTrue(username) = False Or FunctionClass.IsTrue(rndtext)=False  Then
	...
	dr.Read()
        If dr("count001") >0 Then
            dr.Close()
            cmd.CommandText = "update userinfo set active=True,rndtext='" + FunctionClass.Getrndtext(100) + "' where username='" + username + "'"
            If cmd.ExecuteNonQuery() > 0 Then
                Response.Write("激活成功！<br><a href=index.aspx>返回首页</a>")
            Else
